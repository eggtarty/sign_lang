...
def predict(req: PredictRequest):
    if not req.frames: return {"success": False, "prediction": "No Data"}

    # Handle Static (1 Frame)
    if len(req.frames) == 1:
    ...
    # Handle Dynamic (Sequence)
    coord_sequence = [c for f in req.frames if (c := process_base64_to_coords(f)) is not None]
    if len(coord_sequence) < 5: return {"success": False, "prediction": "Keep Moving", "confidence": 0.0}
